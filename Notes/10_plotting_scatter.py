...
with open("NotesA/World firearms murders and ownership - Sheet 1.tsv") as f:
...
firearms_100 = []
names = []
similar = ["Canada", "Norway", "Australia", "Iceland", "Denmark", "Sweden", "Germany", "Japan", "Taiwan", "Singapore", "Netherlands", "Italy", "Spain", "France", "England and Wales", "United States", "South Korea"]
...
plt.scatter(firearms_100, homicide_100k)  # intentional that we want firearms on x and homicides on y

plt.ylabel("homicides per 100k population")
plt.xlabel("firearms per 100 people")
plt.title("Homicides v. Gun Ownership")
plt.grid(color="gray")
...
p = np.polyfit(firearms_100, homicide_100k, 1)  # (x, y, poy_order) 1st order is linear
print(p)  # returns (m, b)
